...
def compile(dirs, judge_client_id,submit_id, problem_id, lang, is_spj):
        ...
        if P.returncode:
            sabo_error_log("info", "Compilation Error for solution {0}, sabo worker: {1}".format(submit_id, getpid()))
            ...
            sabo_error_log("info", "Compilation Pass for solution {0}, sabo worker: {1}".format(submit_id, getpid()))
            if is_spj == '1':
...
def run(submit_id, problem_id, code_path, time_limits, memory_limits, is_spj,
    ...
    result = sabo_core.sabo_core_run(runcfg)
    fin.close()
    fuser.close()
...
def sabo_judge(conf, task, dirs):
    ...
    sabo_change_states(task, judge_map["Compiling"])
    passed, ce_info = compile(dirs,judge_client_id, submit_id, problem_id, lang, logging, log_lock, spj)
    ...
    if passed == False:
        ...
        sabo_change_states(task, res)
    else:
        sabo_change_states(task, judge_map["Running"])
